...
public class ImageHelper {
    ...
    public static BufferedImage createImageFromBoard(byte[][] board) throws IOException, IllegalArgumentException {
        ...
        BufferedImage[] imgs = {readImage("/pictures/pawn.png"), readImage("/pictures/horse.png"), readImage("/pictures/bishop.png"), readImage("/pictures/rook.png"), /* TODO queen */ readImage("/pictures/rook.png"), readImage("/pictures/king.png") /* black */, readImage("/pictures/pawn-1.png/"), readImage("/pictures/horse-1.png"), readImage("/pictures/bishop-1.png"), readImage("/pictures/rook-1.png"), /* TODO queen */ readImage("/pictures/rook-1.png"), readImage("/pictures/king-1.png")};

        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (board[i][j] == 0 || (board[i][j] <= 12 && board[i][j] >= 1)) {
                    if (board[i][j] != 0) {
                        Position pos = getPos(j, i);
                        boardImage = overlayImages(boardImage, imgs[board[i][j] - 1], pos.getX(), pos.getY(), 60, 60);
                    }
    ...
    public static BufferedImage overlayImages(BufferedImage bgImage, BufferedImage fgImage, int x, int y, int sizeX, int sizeY) {
